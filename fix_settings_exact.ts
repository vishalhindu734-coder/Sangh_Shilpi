...
// Assuming the lines are still 3796 to 3924 (0-indexed 3795 to 3923)
// Wait, the line numbers from view_file were 3796 to 3924
const targetContent = lines.slice(3795, 3924).join('\n');
...
if (content.includes(targetContent)) {
  const newCode = content.replace(targetContent, replacement);
  fs.writeFileSync('App.tsx', newCode, 'utf8');
  console.log('Done exact string replacement.');
} else {
  console.log('Target content not found exactly! Dumping string to debug:');
  console.log(JSON.stringify(targetContent.substring(0, 50)));
}
